...
class InvestmentCalc:
    def __init__(self):
        ...
        btCalculate = Button(window, text = "Calculate", command = self.calcValue).grid(row = 6, column = 2, sticky = E)

        window.mainloop()  # create event loop

    def calcValue(self):
        futureValue = self.getFutureValue( float(self.investmentAmountVar.get()), float(self.annualInterestRateVar.get()) / 1200, int(self.yearsVar.get()))
